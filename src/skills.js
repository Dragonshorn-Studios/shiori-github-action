...
export async function generateSkills(documents, taggedIds, config) {
  ...
  await writeGenerated(config.repositoryRoot, STATE_FILE, stableJson({ schemaVersion: 1, tag: config.skillTag, files: generatedFiles.sort() }), []);
...
function isGeneratedPluginSource(source, config) {
  return String(source ?? '').startsWith(`./${config.pluginDirectory}/${GENERATED_PLUGIN_PREFIX}`);
}
